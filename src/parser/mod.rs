 ...
 * not nested (1 space)
  * nested (2 spaces)
   * nested (3 spaces)"#;
    ...
    if let Line::Normal(composite) = &text.lines[1] {
        assert_eq!(composite.style, CompositeStyle::ListItem(0), 
            "Line 1 with 1 space should NOT be nested, got {:?}", composite.style);
    } else {
    ...
    if let Line::Normal(composite) = &text.lines[2] {
        assert_eq!(composite.style, CompositeStyle::ListItem(1), 
            "Line 2 with 2 spaces should be nested, got {:?}", composite.style);
    } else {
    ...
    if let Line::Normal(composite) = &text.lines[3] {
        assert_eq!(composite.style, CompositeStyle::ListItem(1), 
            "Line 3 with 3 spaces should be nested, got {:?}", composite.style);
    } else {
...
fn commonmark_tab_indented_list() {
    ...
    if let Line::Normal(composite) = &text.lines[1] {
        assert_eq!(composite.style, CompositeStyle::ListItem(1), 
            "Tab-indented line should be nested at depth 1, got {:?}", composite.style);
    } else {
        panic!("Line 1 should be a list item");
    }
...
fn commonmark_tab_indented_list_with_different_markers() {
    ...
    if let Line::Normal(composite) = &text.lines[1] {
        assert_eq!(composite.style, CompositeStyle::ListItem(1));
        assert_eq!(composite.compounds[0].src, "Tür von der Garage ins Haus. Muss die immer offen bleiben.");
    } else {
    ...
    if let Line::Normal(composite) = &text.lines[3] {
        assert_eq!(composite.style, CompositeStyle::ListItem(1));
        assert_eq!(composite.compounds[0].src, "Lichtschacht vorne am Eingang. Vergittern");
    } else {
